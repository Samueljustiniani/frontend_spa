...
export class SpaServiceService {
  ...
  list(status?: string): Observable<ServiceEntity[]> {
    ...
    return this.http.get<ServiceEntity[]>(this.API_URL, { params }).pipe(timeout(this.TIMEOUT));
  }
  ...
  getById(id: number): Observable<ServiceEntity> {
    return this.http.get<ServiceEntity>(`${this.API_URL}/${id}`).pipe(timeout(this.TIMEOUT));
  }

  create(service: ServiceEntity): Observable<ServiceEntity> {
    return this.http.post<ServiceEntity>(this.API_URL, service).pipe(timeout(this.TIMEOUT));
  }

  update(id: number, service: ServiceEntity): Observable<ServiceEntity> {
    return this.http.put<ServiceEntity>(`${this.API_URL}/${id}`, service).pipe(timeout(this.TIMEOUT));
  }
